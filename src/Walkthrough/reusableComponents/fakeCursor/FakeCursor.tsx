...
export const FakeCursor: React.FC<{
...
}> = ({ x, y, initialX = 0, initialY = 0, onHover }) => {
  ...
  const cursorX = spring({
    ...
    config: {
      mass: 1,
      damping: 17, // Increase damping for less bounciness
      stiffness: 100, // Decrease stiffness for a softer spring effect
    },
  });
  const cursorY = spring({
    ...
    config: {
      mass: 1,
      damping: 15, // Increase damping for less bounciness
      stiffness: 100, // Decrease stiffness for a softer spring effect
    },
  });

  useEffect(() => {
    // Detect the element under the fake cursor
    if (!isDone && onHover && cursorX >= x && cursorY >= y) {
      onHover(true);
      setIsDone(true);
      // handleClick();
    }
  ...
  // Optional: Trigger `click` event if cursorX and cursorY match the click position
  const handleClick = () => {
    const elements = document.elementsFromPoint(cursorX - 40, cursorY - 20);
    console.dir(elements);
    const topElement = elements[0]; // Get the topmost element under the cursor
    if (topElement) {
      topElement.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    }
  };
